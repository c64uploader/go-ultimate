...
* [`client.Streams.AVISession`](examples/avi_stream.go), [`client.Streams.Audio`](examples/audio_stream.go): Stream video and audio, multiplex to AVI format, or plain audio.
* [`client.Debug`](examples/debug.go): Read and decode C64 state: screen, sprites, registers, memory.
...
* [`c64.NewRAMCartridge`](examples/cartridge_ram.go), [`c64.NewRawCartridge`](examples/cartridge_raw.go): Build CRT cartridge images.
...
* [`c64.ParseT64`](examples/runners.go): Parse .T64 tape archive files and extract PRG entries ready for upload.
...
The project includes a command line tool [`c64ctl`](tools/c64ctl) for accessing `go-ultimate` capabilities. Install it with `go -C tools/c64ctl/ install` or run it directly using `go run github.com/c64uploader/go-ultimate/tools/c64ctl@latest`.
